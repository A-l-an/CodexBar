...
- `codexbar cost` prints token cost usage for Claude, Codex, Cursor, and Antigravity.
  ...
  - `--remote <host>` queries one SSH host's native Codex history alongside this machine; requires a single Codex provider selection. See [SSH Codex cost reports](#ssh-codex-cost-reports).
  - `--summary-only` emits only a native Codex numeric summary as JSON, without daily, model, project, session, or account details. This mode never invokes SSH.
...
### SSH Codex cost reports

Run an explicit, one-shot query with a trusted SSH alias or `user@host`:

```sh
codexbar cost --provider codex --remote research-server
codexbar cost --provider codex --remote research-server --days 7 --format json --pretty
codexbar cost --provider codex --remote research-server --days 7 --refresh
```

Both machines need a CodexBar CLI supporting `--summary-only`. On the remote host, `codexbar` must be available
through `sh -lc`; otherwise the fixed macOS app helper path
`/Applications/CodexBar.app/Contents/Helpers/CodexBarCLI` is selected. Selection happens before execution: a failed
scan does not retry through the fallback. Linux hosts need a runnable CLI and its resource bundle. No installation
or upgrade is performed by this command.

Configure authentication, host-key trust, ports, and jump hosts in your existing SSH configuration. The target
accepts ASCII letters, digits, underscores, dots, and hyphens, optionally separated by one `@`; each component
starts with a letter, digit, or underscore, and the whole target is at most 255 bytes. Use a simple SSH alias for
IPv6 or other connection settings. Authentication is noninteractive, host-key checking is strict, connection
timeout is five seconds, and the SSH process has a sixty-second deadline. Each captured output stream is limited
to 16 KiB. SSH targets, commands, or environment assignments cannot be supplied as a host list.

Reports remain separate: this does not add a combined total, deduplicate overlapping histories, update menu-bar
amounts, or configure polling. Each host uses its own native Codex history, cost-bucket time zone, and pricing
context. Pi/OMP mirrors and OpenCodex are excluded. Amounts are API-equivalent estimates, not subscription bills
or account-wide five-hour/weekly quota measurements. `Today` uses the source's day boundary, and `updatedAt` is the
source snapshot time. Partial coverage and unknown prices remain visible; unknown values do not become zero.

`--refresh` makes one native snapshot request per host with forced refresh. Existing scanner-internal pricing
refresh and unknown-price re-evaluation still apply; the SSH mode adds no retry loop. `--days` retains the ordinary cost command's default of 30 and range
of 1–365. Other providers, repeated `--remote`, `--group-by`, `--breakdown`, or combining `--remote` with
`--summary-only` are rejected before scanning or connecting. Commands without either new flag keep their existing
output contract.

In JSON mode, stdout contains one array of two reports: `host: "local", source: "local"` first, then the validated
SSH target with `source: "ssh"`. Each report has either `summary` or a sanitized `error`. A successful source is
retained even when the other fails, and any source failure makes the command exit non-zero. A valid partial or
unpriced summary is not a transport failure. Connection failures, unsupported remote CLIs, shell banners, invalid
JSON, and oversized responses produce errors rather than zero-usage reports or cached results from an earlier run.

The transport helper can also be called directly on either machine:

```sh
codexbar cost --provider codex --summary-only --format json --days 7
```

It returns an array containing exactly one summary. On a scan failure it returns `[]` with a non-zero exit status.
The summary's required fields are `provider`, `updatedAt`, `bucketTimeZone`, `currencyCode`, `historyDays`,
`historyCoverageIsEstablished`, `provenance`, and `coverage` (`priced`, `unpriced`, `unmetered`, `estimated`).
Optional `sessionTokens`/`sessionCostUSD` describe Today; `last30DaysTokens`/`last30DaysCostUSD` describe the requested
`historyDays` window despite their historical names. Missing numbers mean unknown. `--json`, `--json-only`, and
`--pretty` follow the existing output preferences; summary-only mode requires the resolved format to be JSON.

Only the allowed summary fields are returned across SSH. The command does not copy raw history or add persistent
cross-host state. Existing per-host scan caches and pricing refreshes retain their normal behavior.
